        ...
        if node.current_player=="Max":
            ...
            * The value of node.score and return value should be same
            ...
            for i in node.children:
              new_score = (self.minimax(i,i.current_player))
              scores.append(new_score)
            score = node.score = max(scores)
            return score
...
class Nim(Minimax): # Why Nim takes Minimax? What's the benefit? => You can refer to "Appendix - Python Inheritance" section in this notebook.
    ...
    def max_action(self,node):
        ...
        '''
        for i in node.children:
          scores.append(self.minimax(i,i.current_player))
          # i.score = self.minimax(i,node.current_player)
          new_states.append(i)
    ...
    def minimax_pruning(self, node, current_player, alpha=-1, beta=1):
        ...
        for child in node.children:
            if current_player=="Max":
                ...
                '''
                new_score = self.minimax_pruning(child,child.current_player,alpha,beta)
                scores.append(new_score)
                if (new_score > alpha):
                  alpha = new_score
            ...
            * The value of node.score and return value should be same
            ...
            if alpha >= beta:
              break
        node.score = max(scores) if node.current_player=="Max" else min(scores)
        return node.score
