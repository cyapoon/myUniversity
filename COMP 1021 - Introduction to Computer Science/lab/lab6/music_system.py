...
def updateMusicSummary():
    ...
    if len(music_data) == 0:
        ...
        summary = summary + "song duration = " + str(mins) + "m " + str(secs) + "s," + " instrument=" + music.instrument_list[instrument]
...
def changeInstrument():
    ...
    for instrument in [0,10,19,24,32,40,56,64,73,123]:
        info_for_instr = info_for_instr + str(instrument) + ":" + \
                         music.instrument_list[instrument] + "\n"
...
def adjustSpeed():
    ...
    change = turtle.numinput("Adjust speed",\
                             "Please enter the new speed, in percentage:")
    if change != None: 
        change = int(change)

        for i in range(len(music_data)):
            music_data[i][0] = music_data[i][0] / (change*0.01)
            music_data[i][2] = music_data[i][2] / (change*0.01)
...
def makeCrazyMusic():
    ...
    repetitions = int(turtle.numinput("Number of Repetitions","Please enter the number of repetitions:"))
    duration = int(turtle.numinput("Sound Duration","Please enter the duration of the sound:"))
    starting_pitch = int(turtle.numinput("Starting Pitch","Please enter the starting pitch:"))
    ending_pitch = int(turtle.numinput("Ending Pitch","Please enter the ending pitch:"))

    music_data = []
    x = 0
    for index in range(repetitions):
        for pitch in range(starting_pitch, ending_pitch):
            note = [x, pitch, duration/abs(starting_pitch-ending_pitch)]
            music_data.append(note)
            x = x+(duration/abs(starting_pitch-ending_pitch))
    updateMusicSummary()
