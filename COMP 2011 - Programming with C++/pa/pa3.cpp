...
bool isExist(Course*& head, const char new_code[])  //for Course
{
    if(head == nullptr)
    {
        return false;
...
bool isExist(CourseItem*& head, const char new_code[])  //for CourseItem
{
    if(head == nullptr)
    {
        return false;
    }
    for(CourseItem* p = head; p != nullptr; p = p->next)
    {
        if(strcmp(p->course->code,new_code)==0)
        {
            return true;
        }
    }
...
Course* course_search(Course* head,  const char targetCode[])   //search Course
{
    for(Course* p = head; p != nullptr; p = p->next)
    {
        if(strcmp(p->code,targetCode)== 0)
        {
            return p;
        }
    }
...
bool ll_insert_prerequisite(Course* head, const char targetCode[MAX_CODE], const char preCode[MAX_CODE]) {
    ...
    if(isExist(head, targetCode) && isExist(head,preCode))
    {
        Course* p = course_search(head, targetCode);
        if(isExist(p->prerequisites,preCode))
        {
            return false;
        }

        CourseItem* new_pre = new CourseItem;
        new_pre->course = course_search(head, preCode);
        new_pre->next = nullptr;

        
        
    
        if(p->prerequisites == nullptr || strcmp(p->prerequisites->course->code,new_pre->course->code)>0)
        {
            new_pre->next = p->prerequisites;
            p->prerequisites = new_pre;

        }
        else
        {
            CourseItem* pre_p = p->prerequisites;
            for( ; pre_p->next != nullptr && strcmp(pre_p->next->course->code,new_pre->course->code)<0; pre_p = pre_p->next);
            new_pre->next = pre_p->next;
            pre_p->next = new_pre;
        }
...
bool ll_insert_exclusion(Course* head, const char targetCode[MAX_CODE], const char excludeCode[MAX_CODE]) {
    ...
    if(isExist(head, targetCode) && isExist(head,excludeCode))
    {

        Course* p = course_search(head, targetCode);
        if(isExist(p->exclusions,excludeCode))
        {
            return false;
        }
        
        CourseItem* new_exc = new CourseItem;
        new_exc->course = course_search(head, excludeCode);
        new_exc->next = nullptr;

        
        
    
        if(p->exclusions == nullptr || strcmp(p->exclusions->course->code,new_exc->course->code)>0)
        {
            new_exc->next = p->exclusions;
            p->exclusions = new_exc;

        }
        else
        {
            CourseItem* exc_p = p->exclusions;
            for( ; exc_p->next != nullptr && strcmp(exc_p->next->course->code,new_exc->course->code)<0; exc_p = exc_p->next);
            new_exc->next = exc_p->next;
            exc_p->next = new_exc;
        }
...
bool ll_delete_prerequisite(Course* head, const char targetCode[MAX_CODE], const char preCode[MAX_CODE]) {
    ...
    if(p == nullptr)
    {
        //cout << "targetCode is not found" << endl;
        return false;
    }

    if(isExist(head,targetCode) && isExist(p->prerequisites,preCode))
    {
...
bool ll_delete_exclusion(Course* head, const char targetCode[MAX_CODE], const char excludeCode[MAX_CODE]) {
    ...
    if(isExist(head,targetCode) && isExist(p->exclusions,excludeCode))
    {
...
bool ll_insert_course(Course*& head, const char c[MAX_CODE], const char t[MAX_TITLE], int cred) {
    ...
    if(!isExist(head,c))
    {
        ...
        //special case  the course list is empty or insert to the beginning
        if(head == nullptr || strcmp(head->code, new_course->code)>0)
        {
        ...
        Course* p = head;
        for(; p->next!= nullptr && strcmp(p->next->code,new_course->code)<0; p = p->next);
...
void delete_courseItem(CourseItem* head)
{
    if(head == nullptr)
    {
        return;
    }
    delete_courseItem(head->next);
    delete head;
    head = nullptr;
...
bool ll_delete_course(Course*& head, const char c[MAX_CODE]) {
    ...
    if(isExist(head,c))
    {
        ...
        if(current != nullptr)
        {
            delete_courseItem(current->prerequisites);
            delete_courseItem(current->exclusions);
...
bool ll_modify_course_title(Course* head, const char c[MAX_CODE], const char t[MAX_TITLE]) {
    ...
    {
        Course* p = head;
        for( ; p!= nullptr && strcmp(p->code,c)!= 0; p = p->next);
        if(p!=nullptr)
        {
            strcpy(p->title,t);
            return true;
