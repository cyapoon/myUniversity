...
struct node* send_return(int sockfd, struct sockaddr_in cltaddr, char file_idx, struct node* current, char seq) {
	...
	while(current != NULL)
	{
		unsigned int file_map_copy = current->file_map;
		file_map_copy = (file_map_copy << file_idx) >> 31;
		if (file_map_copy != 0) // find node
		{
			break;
		}
		current = current->next;
...
int check_timeout(long long now, struct rdt3_sender_ctx *ctx_head, int sockfd) {
    ...
    while (current != NULL) {
		...
		if((current->waiting_ack == 1 )&& (now - current->clock > TIMEOUT))
		{
			...
			client.sin_addr.s_addr = current->ip;
			client.sin_port = htons(current->port);
			current->clock = get_current_time();
			send_return(sockfd,client,current->file_idx,current->noack_node,current->noack_num);
		}
