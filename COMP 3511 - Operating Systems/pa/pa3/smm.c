...
void *mm_malloc(size_t size)
{
    ...
    struct meta_data* it = head->next;

    if(it == head)
    {
        it = sbrk(meta_data_size + size);
        it->size = size;
        it->free = 'o';
        list_add_tail(it,head);
        return (void*)((size_t)it+meta_data_size);
    }

    while(it != head)
    {
        if(it->size >= size && it->free == 'f') 
        {
            it->free = 'o';
            break;
        }
    ...
    else if(it->size - size >= meta_data_size)
    {
        struct meta_data* another = (struct meta_data*)((size_t)it + size + meta_data_size);
        struct meta_data temp;
        *another = temp;
        
        another->free = 'f';
        another->size = it->size - size - meta_data_size;
        it->size = size;
        it->free = 'o';
        list_add(another,it,it->next);
...
void mm_free(void *p)
{
    ...
    struct meta_data* it = head->next;
    while((void*)it < p && (void*)it->next < p && it->next!=head) 
    {
        it = it->next;
    }
    it->free = 'f';
