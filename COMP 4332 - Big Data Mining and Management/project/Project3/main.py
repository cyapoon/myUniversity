...
def fetch_restaurant_data(restaurant_name: str) -> Dict:
    ...
    # filter the data for the given restaurant
    restaurant_data = data[data['Restaurant'] == restaurant_name]
    # extract the reviews, ratings and metadata
    reviews = restaurant_data['Review'].tolist()
    ratings = restaurant_data['Rating'].tolist()
    metadata = restaurant_data['Metadata'].tolist()
    return {
...
def calculate_overall_rating(restaurant_name: str, ratings: List[int], followers: List[int]):
    ...
    weighted_ratings = [r * w for r, w in zip(ratings, weights)]
    overall_rating = sum(weighted_ratings) / sum(weights)
...
class IntentionRecognitionAgent:
    ...
    def generate_response(self, user_input: str) -> str:
        ...
        prompt = """
        You are a Intention Recognition Agent. Your task is to recognize user intention and output JSON with 'intention' and 'args'.
        Only return the JSON string with no other text.

        Possible intentions:
        - rating (ask for rating)
        - compare (compare two restaurants) 
        ...
        # TODO: parse the response text to extract the intention and arguments
        # Your code here
...
class ReviewSummaryAgent:
    ...
    def generate_response(self, reviews: List[str]):
        ...
        prompt = """
        You are a Review Summary Agent. Your task is to summarize the reviews into a single string.
        Only return the summary with no other text.

        Example input:
        ["Great food!", "Excellent service", "Good ambiance"]

        Example output:
        The restaurant has great food, excellent service, and good ambiance.
...
class FollowersParserAgent:
    ...
    def generate_response(self, metadata: List[str]):
        ...
        prompt = """
        You are a Followers Parser Agent. Your task is to parse the metadata list into a list of followers integers.
        Only return the list with no other text. 
        The number of values in the list should be equal to the number of metadata strings.

        Example input:
        ["1 Review , 2 Followers", "2 Reviews , 3 Followers", "4 Reviews , 5 Followers"]
